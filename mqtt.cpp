...
TMqtt::TMqtt(void *shared, const char *id, char *host, int port) : mosqpp::mosquittopp(id)
{
	...
	mqttTable["RAM/alarmes/cmd/NivLhGB"] = &TMqtt::alarmeHighGB;
...
void TMqtt::alarmeHighGB(string val)
{
	ram->setAlarmeHighGb(stoi(val));
}

void TMqtt::alarmeHighPB(string val)
{
	ram->setAlarmeHighPb(stoi(val));
}

void TMqtt::alarmeTmpGB(string val)
{
	ram->setAlarmeTmpHighGb(stoi(val));
}
